...
defmodule Ash.Conformance.SQL.Expectations do
  @moduledoc """
  Expectation records for the shipped SQLite and Postgres adapters, which
  share most of them through AshSQL. See `Ash.Conformance.Contracts.Expectations`
  for the rules every record follows.
  """
  ...
  @doc "The records for `:sqlite` or `:postgres`."
  def for(adapter_id) do
    for {id, statuses} <- table(), Map.has_key?(statuses, adapter_id), into: %{} do
